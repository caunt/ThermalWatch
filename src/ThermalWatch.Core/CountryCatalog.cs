...
public static class CountryCatalog
{
    ...
    private static readonly FrozenSet<string> Codes = IsoAlpha3Codes
        .Split(' ', StringSplitOptions.RemoveEmptyEntries)
        .ToFrozenSet(StringComparer.Ordinal);

    private static readonly Lazy<FrozenDictionary<string, string>> DisplayNames = new(CreateDisplayNames);

    public static bool IsValid(string code) => Codes.Contains(code);

    public static string GetDisplayName(string code) =>
        DisplayNames.Value.GetValueOrDefault(code, code);
