...
Every successfully composed notification preview is losslessly re-encoded as a PNG with a kilometre ruler before Core caches it for automatic or manual delivery. The inset bottom and left axes map the complete configured physical coverage from zero through each exact endpoint; their image-border gaps are visual padding rather than omitted distance. The ruler places one-kilometre ticks, labels ten-kilometre intervals and exact endpoints, shows the origin once, and identifies both axes in kilometres. Its anti-aliased sans-serif text has a dark outline, and all text and geometry scale with the shorter pixel dimension. When an unusually dense scale cannot fit every intermediate label, it retains the ticks, origin, units, and exact endpoints and omits only colliding intermediate labels.
