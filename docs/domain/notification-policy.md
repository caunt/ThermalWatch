...
A daytime pass is required by default. Set `NOTIFICATION_REQUIRE_DAYTIME=false` to permit clusters whose representative is from a nighttime pass; disabling the criterion still permits nighttime preview selection and pass-matched imagery.
...
The historical-FRP criterion is enabled by default and is shared by automatic delivery, manual preparation, Viewer eligible-cluster listing, and Viewer diagnostics. It asks whether a current location's total measured FRP substantially exceeds the 95th percentile of its comparable cluster totals from the preceding 30 complete UTC days; it does not infer cause, persistence, or event type.

Core retains each completed day's raw clusters with the immutable history snapshot and builds one radius-specific spatial index over their members for shared historical-FRP evaluation. Replaced immutable histories release their derived indexes after active readers finish. When a completed day contains an anomaly whose deterministic ID is already a member of the current active cluster, Core bypasses that day's indexed clusters, removes the overlapping members, and rebuilds the day. This avoids comparing a current-window observation with itself when the active window crosses a UTC-date boundary and allows the remaining historical members to split into their correct connected components. A historical cluster is spatially comparable when any of its members lies within the configured cluster radius of any current member; the index only narrows possible matches, and the exact Haversine test remains authoritative. The configured time window still governs clustering within a day, but no cross-day time proximity is required for this location match.

Core sorts all spatially matching historical-cluster totals and calculates the inclusive, linearly interpolated 95th percentile at position `(sample count - 1) * 0.95`; one sample therefore supplies its own percentile. The current cluster must have an available total FRP and must be strictly greater than both the historical percentile multiplied by `1.5` and the historical percentile plus `75 MW`. Equality with either derived threshold fails. A historical cluster with no available member FRP is ignored; if no matching cluster has comparable FRP, the criterion passes. A current cluster with no available total FRP fails.

The enabled criterion fails closed with an unavailable outcome until all configured country/source slices are complete and fresh for all 30 prior dates. Today's live bucket is excluded from the baseline even though it is retained by `/api/history`. Disabling `NOTIFICATION_HISTORICAL_FRP_FILTER_ENABLED` reports the criterion as disabled and removes the readiness requirement. See [ADR 0010](../decisions/0010-require-substantial-historical-frp-excess.md) for the current threshold decision.

## Land-cover policy

The land-cover filter is enabled by default and can be disabled with `NOTIFICATION_LAND_COVER_FILTER_ENABLED`. When enabled, the [land-cover policy](../../src/ThermalWatch.Core/NotificationLandCoverPolicy.cs) uses NASA's annual combined MODIS IGBP product. The GIBS client selects the newest year common to every required tile, samples the detection pixels plus pixels intersecting the configured 15 km default proximity around every cluster member, and decodes the official indexed colors into classes.
