...
When Core supplies one or more nearby features, “Possible nearby sources” appears before the satellite details and renders every result as a distance-first monospace line using the fixed `0.00 km` form. Empty or unavailable lookups add no section. Progressive compaction shortens names and less important message detail while retaining all five bounded results within the photo-caption limit.
